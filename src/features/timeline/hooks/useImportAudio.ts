...
/** File types the picker offers. Video pulls the audio track out of a movie. */
const AUDIO_TYPES = [
  'audio/wav',
  'audio/x-wav',
  'audio/mpeg',
...
const VIDEO_TYPES = ['video/mp4', 'video/quicktime', 'video/x-m4v', 'public.movie'];
    ...
    async (source: 'audio' | 'video' = 'audio') => {
      ...
      try {
        const result = await DocumentPicker.getDocumentAsync({
          type: source === 'video' ? VIDEO_TYPES : AUDIO_TYPES,
